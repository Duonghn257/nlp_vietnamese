...
def test_generation(model, tokenizer, device, test_cases=None):
    ...
    for i, prompt in enumerate(test_cases, 1):
        ...
        generation_configs = [
            ...
            {'temperature': 0.3, 'top_k': 10, 'top_p': 1.0, 'max_new_tokens': 15, 'name': 'Conservative'}
        ]
...
def main():
    ...
    print("input_ids = torch.tensor([tokenizer.encode(prompt, add_special_tokens=False)], device=device)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train Vietnamese Text Generation Model')
    parser.add_argument('--data_file', type=str, default='data/truyen_kieu.txt',
